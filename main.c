...
// Here we are printing that random word from the word[] string.
void generateRandomSentence(char **words, int numWordsToGenerate, int numWords)
{
	printf("\nOriginal Sentence: ");
...
// Here we raed the leaderboard(records) from the file to be used in the display function.
void readingLeaderboard()
{
...
// Here we do the processing of changing the theme!
void ActuallyChangingTheme(char bg_colour, char text_colour)
{
...
void InputtingNewTheme()
{
	...
	// Validate input
	if ((bg_colour >= '0' && bg_colour <= '9') || (bg_colour >= 'A' && bg_colour <= 'F') && (text_colour >= '0' && text_colour <= '9') || (text_colour >= 'A' && text_colour <= 'F'))
	{
